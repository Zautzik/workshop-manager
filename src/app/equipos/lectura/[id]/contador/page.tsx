 ...
 *
 * Antes esto vivía directo en /equipos/lectura/[id] -- el QR pegado en el
 * equipo abría el formulario sin pasos intermedios. Se movió un nivel
 * abajo cuando esa pantalla pasó a ser un menú con dos caminos (lectura o
 * mantenciones pendientes, ver el page.tsx del padre) para no imprimir un
 * segundo QR por máquina. El formulario en sí no cambió.
 *
 * Está pensada para un teléfono en el taller: un dato, botones grandes, y
 * listo. Todo lo que no sea el número sobra.
 *
  ...
  return (
    <div className="mx-auto max-w-md space-y-6 p-5">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-bold leading-tight">Anotar lectura</h1>
...
export default function ContadorPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return (
