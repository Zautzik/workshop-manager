...
const groups = [
  {
    ...
    items: [
      { label: 'MÃ¡quinas',     description: 'Registro, ficha tÃ©cnica y estado de cada equipo',           icon: Cpu,         href: '/maintenance/maquinas',  color: 'bg-orange-500/10 text-orange-400'   },
      { label: 'Plan & Ã“rdenes',description: 'OTs de mantenimiento, programa semanal y checklists',      icon: ClipboardList,href: '/maintenance/ordenes',   color: 'bg-sky-500/10 text-sky-400'        },
      { label: 'EjecuciÃ³n',    description: 'Registro de intervenciones y trabajos realizados',           icon: Wrench,      href: '/maintenance/ejecucion', color: 'bg-fuchsia-500/10 text-fuchsia-400'},
    ],
  },
  {
    label: 'AnÃ¡lisis & Predictivo',
    color: 'from-violet-500/10 to-indigo-500/5',
    border: 'border-violet-500/20',
    heading: 'text-violet-400',
    items: [
      { label: 'Historial & KPIs',    description: 'Registro histÃ³rico, MTBF, MTTR y mÃ©tricas de flota',   icon: BarChart3,   href: '/maintenance/historial', color: 'bg-indigo-500/10 text-indigo-400'},
...
export default function MaintenancePage() {
  return (
    <ProtectedRoute allowedRoles={['admin', 'technician', 'supervisor', 'manager']}>
      <ModuleHexLanding
        title="Equipos"
        subtitle="Mantenimiento, Ã³rdenes de trabajo y gestiÃ³n de la flota"
        groups={groups}
      />
    </ProtectedRoute>
  );
