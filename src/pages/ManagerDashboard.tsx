...
const ManagerDashboard = () => {
	...
	const fetchData = async () => {
		...
		const inProgress =
			jobsData?.filter(j => j.status === 'in_progress').length || 0;
			...
			<header className='border-b border-manager/20 shadow-lg sticky top-0 z-50 backdrop-blur-sm bg-card/95'>
				<div className='container mx-auto px-4 py-4 flex items-center justify-between'>
					<div className='flex items-center gap-4'>
						...
						<div>
							<h1 className='text-2xl font-bold text-manager'>
								{t('managerDashboard')}
							</h1>
			...
			<main className='container mx-auto px-4 py-8 space-y-8'>
				<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
					<Card className='border-manager/20 hover:border-manager/40 transition-all hover:shadow-lg hover:-translate-y-1 duration-300'>
						...
						<CardContent>
							<div className='text-4xl font-bold text-manager'>
								{stats.total}
							</div>
					...
					<Card className='border-supervisor/20 hover:border-supervisor/40 transition-all hover:shadow-lg hover:-translate-y-1 duration-300'>
						...
						<CardContent>
							<div className='text-4xl font-bold text-supervisor'>
								{stats.completed}
							</div>
					...
					<TabsContent value='overview' className='space-y-8'>
						<div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
							<Card className='border-manager/20'>
								<CardHeader>
									<CardTitle className='text-manager'>
										{t('machines')}
									</CardTitle>
								</CardHeader>
								<CardContent>
												...
												<span
													className={`px-3 py-1 rounded-full text-xs font-medium ${
														machine.status === 'running'
															? 'bg-supervisor/20 text-supervisor'
