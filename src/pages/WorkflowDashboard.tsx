...
export default function WorkflowDashboard() {
	...
	const fetchAssignments = async () => {
		const { data, error } = await supabase
			...
			.select(
				`*, worker:workers(*), workstation:workstations(*), shift:shifts(*)`
			)
	...
	const handleDragEnd = async (event: DragEndEvent) => {
		...
		if (!selectedShiftId) {
			toast({
				...
				description:
					'Choose a shift in the Layout settings before assigning workers.',
				variant: 'destructive',
			});
			return;
		}
		...
		const currentAssignments = assignments.filter(
			a => a.workstation_id === workstation.id
		);
					...
					<div className='flex items-center gap-4'>
						...
						<div>
							...
							<p className='text-muted-foreground'>
								Dynamic Workflow & Performance Tracking
							</p>
						</div>
					</div>
						...
						<div className='flex items-center justify-between'>
							<div>
								<h3 className='text-lg font-bold text-foreground'>
									Active OT: {selectedOT.ot_number}
								</h3>
								<p className='text-sm text-muted-foreground'>
									{selectedOT.client_name} - {selectedOT.quantity} units
								</p>
							</div>
							<Button
								variant='outline'
									...
									<div className='flex gap-2 flex-wrap justify-end'>
										{shifts.map(shift => (
											<Button
												...
												variant={
													selectedShiftId === shift.id ? 'default' : 'outline'
												}
												className={
