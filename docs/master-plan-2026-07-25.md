...
| L1.3 | Verify `NEXT_PUBLIC_DEV_BYPASS` off in production | Vercel | smoke green vs prod build | S | **DONE 2026-09-16.** Checked Vercel dashboard, both Production and Preview: neither `NEXT_PUBLIC_DEV_BYPASS` nor `NODE_ENV` exists as a project variable in either environment — nothing to clear. `scripts/verify-node-env.cjs` (added 2026-09-15, wired as `prebuild`) now fails the build outright if `NODE_ENV=development` ever shows up in a deployed build, closing this permanently rather than needing a re-check |
