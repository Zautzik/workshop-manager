-- ============================================================
-- C1.2 — convert_vb_to_ot(): a signed Visto Bueno becomes a prefilled OT,
-- and its estimate freezes into the cost ledger. One tap, zero re-entry.
-- ============================================================

ALTER TABLE public.ots ADD COLUMN IF NOT EXISTS vb_id UUID
  REFERENCES public.vistos_buenos(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_ots_vb ON public.ots(vb_id);
...
BEGIN
  ...
  INSERT INTO public.ots (
    ...
    v_vb.product_name, v_vb.product_type, 'normal', v_vb.quantity,
    COALESCE(v_vb.notes, v_vb.product_name),
    v_vb.width_cm, v_vb.height_cm, v_vb.substrate_type, v_vb.grammage_gsm,
    v_vb.color_front, v_vb.color_back,
