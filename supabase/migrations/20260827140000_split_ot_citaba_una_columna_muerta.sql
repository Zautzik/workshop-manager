-- `split_ot` citaba una columna que ya no existe
--
-- Dividir una OT desde el tablero fallaba con
--
--     column "current_workstation_id" of relation "ots" does not exist
--
-- La columna se eliminó el 2026-08-02 en `retirar_workstations`: los puestos de
-- trabajo se habían fusionado con las máquinas y `ots.current_workstation_id`
-- estaba en 0 de 2 filas, así que borrarla era correcto. Lo que no se revisó fue
-- quién la seguía nombrando — y `split_ot`, escrita en junio, la copiaba de la
-- OT madre al fragmento.
--
-- ── Por qué no saltó hasta hoy ──────────────────────────────────────────────
--
-- Es la trampa de PL/pgSQL: el cuerpo de una función se guarda como TEXTO y sus
-- referencias se resuelven recién al ejecutarla. `DROP COLUMN` no revisa las
-- funciones que la mencionan y no falla; tampoco falla el despliegue, ni el
-- arranque, ni ninguna otra consulta. La función queda rota y en silencio, y se
-- entera el primero que intenta usarla — en este caso, alguien partiendo un
-- trabajo en el Kanban tres semanas y media después.
--
-- Es la misma familia de defecto que ya dejó cicatriz dos veces en este
-- repositorio: el disparador de turnos que seguía buscando `worker_id`, y la
-- línea de tiempo de costo que citaba seis columnas muertas. Las tres veces el
-- daño no fue el borrado sino no haber preguntado quién más nombraba lo borrado.
--
-- ── Qué se revisó esta vez ──────────────────────────────────────────────────
--
-- Las 62 columnas que `split_ot` inserta, contra el esquema real: sólo esa
-- sobraba. El resto del cuerpo queda idéntico — los seguros de concurrencia,
-- el cálculo de etiqueta y el tope de 26 fragmentos no se tocan.
...
BEGIN
  ...
  INSERT INTO public.ots (
    ...
    -- `current_workstation_id` iba acá. Los puestos son máquinas desde agosto:
    -- `assigned_machine_id` es el único dato de dónde se hace el trabajo.
